...
impl<T: HandleType> Handle<T> {
  ...
  pub fn set(&mut self, value: *mut c_void, size: c_uint, attrtype: types::Attr, err: &Handle<OCIError>) -> Result<()> {
  ...
  pub fn set_str(&mut self, value: &str, attrtype: types::Attr, err: &Handle<OCIError>) -> Result<()> {
    self.set(value.as_ptr() as *mut c_void, value.len() as c_uint, attrtype, err)
  }
  /// Устанавливает хендл-атрибут хендлу
  pub fn set_handle<U: HandleType>(&mut self, value: &Handle<U>, attrtype: types::Attr, err: &Handle<OCIError>) -> Result<()> {
...
impl<'d, T: 'd + DescriptorType> Descriptor<'d, T> {
  ...
  pub unsafe fn get(&self, value: *mut c_void, size: &mut c_uint, attrtype: types::Attr, err: &Handle<OCIError>) -> Result<()> {
  ...
  pub unsafe fn set(&self, value: *mut c_void, size: c_uint, attrtype: types::Attr, err: &Handle<OCIError>) -> Result<()> {
    //FIXME: Использование данной функции требует реализовать AttrHandle для всех, кто реализует DescriptorType.
    // Но к сожалению, попытка сделать это дает невнятную ошибку о том, что тип-донор для ID может не жить достаточно долго(!)
    //unsafe { attr_set(self.native, value, size, attrtype, err) }
    let res = OCIAttrSet(
  ...
  pub fn get_c_uint(&self, attrtype: types::Attr, err: &Handle<OCIError>) -> Result<c_uint> {
  ...
  pub fn get_str(&self, attrtype: types::Attr, err: &Handle<OCIError>) -> Result<String> {
