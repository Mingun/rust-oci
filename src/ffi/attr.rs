...
//! типа нет, то значит данный атрибут нельзя читать или писать соответственно.
...
/// содержащие, соответственно, атрибуты, доступные для чтения и для записи. Макрос может вызываться
/// в двух формах: с одним списком и с двумя. В первом случае все атрибуты считаются доступными как
/// для чтения, так и для записи. Во втором случае первый список определяет атрибуты, доступные для
/// чтения, а второй -- доступные для записи.
#[macro_export]
macro_rules! enum_ {
  // Чтение и запись всего
  (
    ($($use_:item)*)
    $name:ident,
    ($($rw:ident($trw:ty) = $id_rw:expr),+)
  ) => (
    pub mod $name {
      $($use_)*
  ...
  (
    ($($use_:item)*)
    $name:ident,
    ($($r:ident($tr:ty) = $id_r:expr),+)
    ($($w:ident($tw:ty) = $id_w:expr),+)
  ) => (
    pub mod $name {
      $($use_)*
...
pub enum Param {
  ...
  OCI_ATTR_INDEX_ONLY = ,
  // Только процедур, функций и подпрограмм
