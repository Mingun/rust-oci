...
impl FromDB for NaiveDate {
  fn from_db(ty: Type, raw: &[u8], conn: &Connection) -> Result<Self> {
    match ty {
      ...
      Type::TIMESTAMP_TZ => {
        // Наивное время из метки времени с часовым поясом получаем в поясе UTC -- таким образом, одинаковые метки времени будут
        // одинаковыми при извлечении из базы, даже если внутри они хранятся по разному (00:00:00 +00:00 === 05:00:00 +05:00)
        // после извлечения значение наивного времени в обоих случаях будет 00:00:00 -- равному значению в поясе +00:00 (UTC).
